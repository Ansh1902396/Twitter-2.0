...
function Tweetbox({setTweets} : Props) {
    ...
    const postTweet  = async()=>{
      const tweetInfo : TweetBody = {
        ...
        profileimg :session?.user?.image||'https://pbs.twimg.com/profile_images/1622253557415284742/6ctU3L2f_400x400.jpg',
        image : image, 
      }
  ...
  return (
    <div className="flex space-x-2 p-5">
      <img
        ...
        src={session?.user?.image ||"https://pbs.twimg.com/profile_images/1622253557415284742/6ctU3L2f_400x400.jpg"}
        alt=""
      />
        ...
        <form action="" className="flex flex-1 flex-col">
          ...
          <div className="flex items-center">
            <div className="flex space-x-2 flex-1">
              ...
              <PhotoIcon onClick = {()=>{setImageUrlBoxIsOpen(!imageUrlBoxIsOpen)}} className="h-5 w-5 text-twitter cursor-pointer transition-transform duration-150 ease-out hover:scale-150" />
            ...
            <button onClick ={handleSubmit} disabled =  {!input} className="bg-twitter px-5 py-2 text-white font-bold rounded-full disabled:opacity-40">
              tweet
            </button>
          </div>
          ...
          {imageUrlBoxIsOpen && (
              <form className="rounded-lg mt-5 flex bg-twitter/80 py-2 px-4">
                <input ref = {imageInputReference} type="text" placeholder="enter image url" className="flex-1 bg-transparent p-2 text-white outline-none placeholder: text-white"  />
